...
| --- | --- | --- |
| Estimate diffing | "What changed since last quote, and which meter caused it?" is the first question every reviewer asks. Freeze exports already pin everything needed to diff two runs. | Anyone re-quotes an account more than once. — **Shipped (2026-08-12):** `diffEstimates` (pure, in `shared/lib/estimate-diff.ts`) classifies each meter added/removed/changed/unchanged with before/after and a signed delta, biggest mover first, aggregating a meter that bills on more than one line (GCP double egress) and treating sub-cent moves as unchanged. The `EstimateDiff` widget renders only what moved (nothing on a no-op), and `EstimatorPage` records each run against the previous one, so re-running shows "what changed since your last estimate" with a signed total. 15 tests (core + widget). |
| Confidence-weighted totals | A total that mixes a verified Event Hubs line with an invented ACR line reads as one number. Weighting, or splitting the total into *vendor-backed* and *modelled*, keeps the honesty visible in the headline. | Modelled capabilities stay in the product. — **Shipped (2026-08-12), reframed:** the vendor-backed/modelled split is moot now that every meter is vendor-backed, but the *estimator's* declared confidence still varies (audit High, ADS Outpost Low). So the breakdown now shows a **confidence-band split** — "of $X, High $A · Med $B · Low $C" (`confidenceBandTotals`, folds unknown into Low so the total always reconciles), shown only when more than one band carries money. |
| Per-line "show the arithmetic" | The notes now carry object and operation counts; rendering them under each line removes the last reason to read the source. | REQ-1 ships. — **Partly shipped (2026-08-11):** the cost breakdown now shows a per-line **Source** column — a "✓ verified" link to the official vendor page for a vendor-backed rate, or the ledger verdict for one that isn't (`CostBreakdown`, `MeterVerification` was already on the response). The arithmetic *notes* (object/op counts) still need a `LineItem.notes` field added to the OpenAPI contract + engine passthrough before the UI can render them — **deferred (2026-08-12):** investigation found the sub-estimators build `notes[]` but `create-estimate` **discards** them (they never reach `LineItem` or the response), so surfacing is moderate cross-cutting plumbing (engine attach + contract + regen + UI) for modest gain over the debug log, which already prints the arithmetic (`DEBUG=cost:dspm`). Not worth it ahead of estimate diffing. |
